...
git clone  --recursive --branch release/17.x https://github.com/llvm/llvm-project.git
cd ..
...
cmake_flags="-DCMAKE_INSTALL_PREFIX=$llvm_installdir -DLLVM_ENABLE_ASSERTIONS=OFF -DLLVM_FORCE_ENABLE_STATS=ON -DCMAKE_BUILD_TYPE=$llvm_build_type -DLLVM_ENABLE_Z3_SOLVER=OFF  -DLLVM_ENABLE_PROJECTS=clang-tools-extra;lld;llvm;clang;compiler-rt"
