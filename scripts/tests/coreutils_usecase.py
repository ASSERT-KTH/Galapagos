...
if __name__ == "__main__":
    # Compile openssl
    print(f"Results will be in {os.path.abspath('out/coreutils')}")
    logging.basicConfig(level=logging.DEBUG)

    verif = verifier.AliveVerifier(debug=True)

    async def compare(original_folder, shadow1, original: usecases.coreutils.coreutils, variant: usecases.coreutils.coreutils):
        ...
        def save(ot):
            function_folder = f"out/coreutils/{variant.function_name}"
    ...
    async def main():
        ...
        coreutils_project_folder = "../../use_cases/coreutils"

        coreutils_project_folder = os.path.join(DIRNAME, coreutils_project_folder)
        original_uc = usecases.coreutils.coreutils("all", None, None, None, 0, 0, "coreutils", doreplace = False)

        # UNcomment this for real
        # original_uc.compiled = True
        # original_uc.tested = True

        shadow_original = await original_uc.shadow(coreutils_project_folder, name="coreutilst1")
        await original_uc.compile(shadow_original)

        # Reading the functions and the variants
        # Reading json inside ../../functions/<library>/functions_info.json
        test_cases = []
        WORKSPACE = os.path.join(DIRNAME, "../../functions/coreutils")
            ...
            for idx, function in enumerate(functions_info):
                ...
                for variant_of_function in os.listdir(f"{WORKSPACE}/variants"):
                    ...
                    if funcname == f"{idx}_{function['name']}":
                        ...
                        testcase = usecases.coreutils.coreutils(
                                funcname,
                                original_project_folder=coreutils_project_folder,
        ...
        for test in test_cases:
            tasks.append(asyncio.create_task(compare(os.path.join(DIRNAME, "../../use_cases/coreutils"),shadow_original, original_uc, test)))
            break
