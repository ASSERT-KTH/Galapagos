...
import asyncio
from usecases.miscompilation import is_executable, CLONE_PATH
import verifier
import logging
import json
import os
...
if __name__ == "__main__":
    ...
    async def main():
        ...
        project_folder = f"../../miscompilation/{LIBRARY}/project"

        project_folder = os.path.join(DIRNAME, project_folder)

        uc_name = f"usecases.miscompilation"
        uc_name = __import__(uc_name).miscompilation
        ...
        test_cases = []
        WORKSPACE = os.path.join(DIRNAME, f"../../miscompilation/{LIBRARY}/function")
