...
if __name__ == "__main__":
    # Compile openssl
    print(f"Results will be in {os.path.abspath('out/libsodium')}")
    logging.basicConfig(level=logging.DEBUG)

    verif = verifier.AliveVerifier(debug=True)

    async def compare(original_folder, shadow1, original: usecases.libsodium.libsodium, variant: usecases.libsodium.libsodium):
    ...
    async def main():
        ...
        libsodium_project_folder = "../../use_cases/libsodium"

        libsodium_project_folder = os.path.join(DIRNAME, libsodium_project_folder)
        original_uc = usecases.libsodium.libsodium("all", None, None, None, 0, 0, "libsodium", doreplace = False)

        # UNcomment this for real
        # original_uc.compiled = True
        # original_uc.tested = True

        shadow_original = await original_uc.shadow(libsodium_project_folder, name="libsodiumt1")
        ...
        for test in test_cases:
            tasks.append(asyncio.create_task(compare(os.path.join(DIRNAME, "../../use_cases/libsodium"),shadow_original, original_uc, test)))
            #break
