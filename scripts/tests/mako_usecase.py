...
if __name__ == "__main__":
    # Compile openssl
    print(f"Results will be in {os.path.abspath('out/mako')}")
    logging.basicConfig(level=logging.DEBUG)

    verif = verifier.AliveVerifier(debug=True)

    async def compare(original_folder, shadow1, original: usecases.mako.mako, variant: usecases.mako.mako):
        ...
        def save(ot):
            function_folder = f"out/mako/{variant.function_name}"
    ...
    async def main():
        ...
        mako_project_folder = "../../use_cases/mako"

        mako_project_folder = os.path.join(DIRNAME, mako_project_folder)
        original_uc = usecases.mako.mako("all", None, None, None, 0, 0, "mako", doreplace = False)

        # UNcomment this for real
        # original_uc.compiled = True
        # original_uc.tested = True

        shadow_original = await original_uc.shadow(mako_project_folder, name="makot1")
        ...
        test_cases = []
        WORKSPACE = os.path.join(DIRNAME, "../../functions/mako")
            ...
            for idx, function in enumerate(functions_info):
                ...
                for variant_of_function in os.listdir(f"{WORKSPACE}/variants"):
                    ...
                    if funcname == f"{idx}_{function['name']}":
                        ...
                        testcase = usecases.mako.mako(
                                funcname,
                                original_project_folder=mako_project_folder,
        ...
        for test in test_cases:
            tasks.append(asyncio.create_task(compare(os.path.join(DIRNAME, "../../use_cases/mako"),shadow_original, original_uc, test)))
            break
