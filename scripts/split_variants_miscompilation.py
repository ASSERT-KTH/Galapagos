...
from os import path
import sys
from generate_variants_rq3 import WORKSPACE
import json
...
def split(info, function_path, lang):
    for i, fn in enumerate(info):
        with open(path.join(function_path, 'variants', lang, f'{fn["name"]}.c.variants')) as fn_file:
            data = fn_file.read()
            ...
            while start != -1:
                ...
                while count > 0:
                    ...
                    if data[end] == '}':
                        count -= 1
                with open(path.join(function_path, 'variants', lang, f'{fn["name"]}_{it}.{lang}.raw'), 'w') as out:
