; ModuleID = '/home/javier/Galapagos/functions/alsa-lib/variants/go/0_alaw_to_s16_3.bc'
...
module asm "\09.ascii \22)\22"
module asm "\09.ascii \22 <inl:779>\\n\22"
module asm "\09.ascii \22 // /home/javier/Galapagos/functions/alsa-lib/variants/go/0_alaw_to_s16_3.go:3\\n aVal = (aVal ^ $convert(<type -20>, 85 )) //4\\n var t <type -3> = ($convert(<type -3>, (aVal & $convert(<type -20>, 127 ))) << $convert(<type -12>, 3 )) //5\\n { //6\\n  if (t < $convert(<type -3>, 128 )) { //6\\n   t = (t + $convert(<type -3>, 8 )) //7\\n  } else { //8\\n   t = (t + ($convert(<type -3>, 2048 ) + ((t & $convert(<type -3>, 112 )) << $convert(<type -12>, 4 )))) //9\\n   t = (t << (((t >> $convert(<type -12>, 4 )) & $convert(<type -3>, 7 )) - $convert(<type -3>, 1 ))) //10\\n  } //6\\n } //11\\n { //12\\n  if ((aVal & $convert(<type -20>, 128 )) != $convert(<type -20>, 0 )) { //12\\n   { //13\\n    $ret0 = t //13\\n    return //13\\n   } //0\\n  } //12\\n } //14\\n { //15\\n  $ret0 = -t //15\\n  return //15\\n } //0\\n\22"
module asm "\09.ascii \22checksum C6B6530EA3AA6CAB553FD1D1CD65579438137A4F\\n\22"
module asm "\09.text"
...
; Function Attrs: null_pointer_is_valid
define i32 @main.alawToS16(ptr nest nocapture readnone %nest.0, i8 zeroext %aVal) local_unnamed_addr #0 {
entry:
  %xor.0 = and i8 %aVal, 127
  %iand.0 = xor i8 %xor.0, 85
  %zext.0 = zext i8 %iand.0 to i32
  %shl.0 = shl nuw nsw i32 %zext.0, 3
  %icmp.2 = icmp ult i8 %iand.0, 16
  br i1 %icmp.2, label %then.0, label %else.0

then.0:                                           ; preds = %entry
  %add.0 = add nuw nsw i32 %shl.0, 8
  br label %fallthrough.0

fallthrough.0:                                    ; preds = %fallthrough.2, %then.0
  %t.0 = phi i32 [ %add.0, %then.0 ], [ %shl.2, %fallthrough.2 ]
  ...
  %iand.1 = shl nuw nsw i32 %zext.0, 7
  %shl.1 = and i32 %iand.1, 1792
  %add.1 = or i32 %shl.1, 2048
  %add.2 = add nuw nsw i32 %add.1, %shl.0
  %0 = lshr i32 %add.2, 4
  %iand.2 = and i32 %0, 7
  %icmp.0 = icmp eq i32 %iand.2, 0
  br i1 %icmp.0, label %then.1, label %fallthrough.2

then.1:                                           ; preds = %else.0
  call void @runtime.panicshift(ptr nest undef)
  unreachable

fallthrough.2:                                    ; preds = %else.0
  %sub.0 = add nsw i32 %iand.2, -1
  %shl.2 = shl i32 %add.2, %sub.0
  br label %fallthrough.0
}

; Function Attrs: noreturn null_pointer_is_valid
declare void @runtime.panicshift(ptr) local_unnamed_addr #1

; Function Attrs: mustprogress nofree norecurse nosync nounwind null_pointer_is_valid willreturn memory(none)
define void @main.main(ptr nest nocapture readnone %nest.1) local_unnamed_addr #2 {
entry:
  ret void
}
...
then.10:                                          ; preds = %else.9
  ...
  %tmpv.13.sroa.0.0.cast.10.sroa_idx = bitcast ptr %key1 to ptr
  %tmpv.13.sroa.0.0.copyload = load i64, ptr %tmpv.13.sroa.0.0.cast.10.sroa_idx, align 8
  %tmpv.14.sroa.0.0.cast.15.sroa_idx = bitcast ptr %key2 to ptr
  %tmpv.14.sroa.0.0.copyload = load i64, ptr %tmpv.14.sroa.0.0.cast.15.sroa_idx, align 8
  %icmp.10.not = icmp eq i64 %tmpv.13.sroa.0.0.copyload, %tmpv.14.sroa.0.0.copyload
...
label.0:                                          ; preds = %then.23
  %add.3 = add nuw nsw i64 %tmpv.27.018, 1
...
then.23:                                          ; preds = %label.0, %entry.split
  %tmpv.27.018 = phi i64 [ %add.3, %label.0 ], [ 0, %entry.split ]
  %tmpv.29.sroa.4.0.cast.24.sroa_idx7 = getelementptr [61 x { i32, i64, i64 }], ptr %cast.20, i64 0, i64 %tmpv.27.018, i32 2
  %tmpv.29.sroa.4.0.copyload = load i64, ptr %tmpv.29.sroa.4.0.cast.24.sroa_idx7, align 8
  %tmpv.29.sroa.3.0.cast.24.sroa_idx6 = getelementptr [61 x { i32, i64, i64 }], ptr %cast.20, i64 0, i64 %tmpv.27.018, i32 1
  %tmpv.29.sroa.3.0.copyload = load i64, ptr %tmpv.29.sroa.3.0.cast.24.sroa_idx6, align 8
  %tmpv.29.sroa.0.0.cast.24.sroa_idx = getelementptr [61 x { i32, i64, i64 }], ptr %cast.20, i64 0, i64 %tmpv.27.018, i32 0
  %tmpv.29.sroa.0.0.copyload = load i32, ptr %tmpv.29.sroa.0.0.cast.24.sroa_idx, align 8
  %tmpv.30.sroa.0.0.cast.29.sroa_idx = getelementptr [61 x { i32, i64, i64 }], ptr %cast.21, i64 0, i64 %tmpv.27.018, i32 0
  %tmpv.30.sroa.0.0.copyload = load i32, ptr %tmpv.30.sroa.0.0.cast.29.sroa_idx, align 8
  %tmpv.30.sroa.3.0.cast.29.sroa_idx4 = getelementptr [61 x { i32, i64, i64 }], ptr %cast.21, i64 0, i64 %tmpv.27.018, i32 1
  %tmpv.30.sroa.3.0.copyload = load i64, ptr %tmpv.30.sroa.3.0.cast.29.sroa_idx4, align 8
  %tmpv.30.sroa.4.0.cast.29.sroa_idx5 = getelementptr [61 x { i32, i64, i64 }], ptr %cast.21, i64 0, i64 %tmpv.27.018, i32 2
  %tmpv.30.sroa.4.0.copyload = load i64, ptr %tmpv.30.sroa.4.0.cast.29.sroa_idx5, align 8
  %icmp.21 = icmp eq i32 %tmpv.29.sroa.0.0.copyload, %tmpv.30.sroa.0.0.copyload
  %icmp.22 = icmp eq i64 %tmpv.29.sroa.3.0.copyload, %tmpv.30.sroa.3.0.copyload
  %tmpv.36.0.in = select i1 %icmp.21, i1 %icmp.22, i1 false
  %icmp.23 = icmp eq i64 %tmpv.29.sroa.4.0.copyload, %tmpv.30.sroa.4.0.copyload
  %tmpv.37.0.in = select i1 %tmpv.36.0.in, i1 %icmp.23, i1 false
  br i1 %tmpv.37.0.in, label %label.0, label %common.ret
}
...
label.0:                                          ; preds = %then.38
  %add.4 = add nuw nsw i64 %tmpv.54.011, 1
...
label.0.1:                                        ; preds = %label.0
  %add.4.1 = add nuw nsw i64 %tmpv.54.011, 2
...
label.0.2:                                        ; preds = %label.0.1
  %add.4.2 = add nuw nsw i64 %tmpv.54.011, 3
...
then.38:                                          ; preds = %label.0.2, %entry.split
  %tmpv.54.011 = phi i64 [ %add.4.2, %label.0.2 ], [ 0, %entry.split ]
  %index.2 = getelementptr [33 x double], ptr %cast.40, i64 0, i64 %tmpv.54.011
  %.index.ld.0 = load double, ptr %index.2, align 8
  %index.3 = getelementptr [33 x double], ptr %cast.41, i64 0, i64 %tmpv.54.011
...
then.43:                                          ; preds = %then.46
  %icmp.49 = icmp eq ptr %tmpv.70.sroa.0.0.copyload, %tmpv.72.sroa.0.0.copyload
  br i1 %icmp.49, label %else.45, label %fallthrough.43

fallthrough.43:                                   ; preds = %then.43
  %call.17 = call i32 @memcmp(ptr %tmpv.70.sroa.0.0.copyload, ptr %tmpv.72.sroa.0.0.copyload, i64 %tmpv.70.sroa.3.0.copyload)
...
common.ret:                                       ; preds = %then.46, %else.45, %fallthrough.43
  ...
  %add.5 = add nuw nsw i64 %tmpv.68.019, 1
  %exitcond.not = icmp eq i64 %add.5, 27
  br i1 %exitcond.not, label %common.ret, label %then.46

then.46:                                          ; preds = %else.45, %entry.split
  %tmpv.68.019 = phi i64 [ %add.5, %else.45 ], [ 0, %entry.split ]
  %tmpv.70.sroa.0.0.cast.48.sroa_idx = getelementptr [27 x { ptr, i64 }], ptr %cast.44, i64 0, i64 %tmpv.68.019, i32 0
  %tmpv.70.sroa.0.0.copyload = load ptr, ptr %tmpv.70.sroa.0.0.cast.48.sroa_idx, align 8
  %tmpv.70.sroa.3.0.cast.48.sroa_idx3 = getelementptr [27 x { ptr, i64 }], ptr %cast.44, i64 0, i64 %tmpv.68.019, i32 1
  %tmpv.70.sroa.3.0.copyload = load i64, ptr %tmpv.70.sroa.3.0.cast.48.sroa_idx3, align 8
  %tmpv.72.sroa.0.0.cast.51.sroa_idx = getelementptr [27 x { ptr, i64 }], ptr %cast.45, i64 0, i64 %tmpv.68.019, i32 0
  %tmpv.72.sroa.0.0.copyload = load ptr, ptr %tmpv.72.sroa.0.0.cast.51.sroa_idx, align 8
  %tmpv.72.sroa.3.0.cast.51.sroa_idx1 = getelementptr [27 x { ptr, i64 }], ptr %cast.45, i64 0, i64 %tmpv.68.019, i32 1
  %tmpv.72.sroa.3.0.copyload = load i64, ptr %tmpv.72.sroa.3.0.cast.51.sroa_idx1, align 8
  %icmp.48 = icmp eq i64 %tmpv.70.sroa.3.0.copyload, %tmpv.72.sroa.3.0.copyload
