; ModuleID = '/home/javier/Galapagos/functions/alsa-lib/variants/go/3_ulaw_to_s16_2.bc'
...
module asm "\09.ascii \22)\22"
module asm "\09.ascii \22 <inl:544>\\n\22"
module asm "\09.ascii \22 // /home/javier/Galapagos/functions/alsa-lib/variants/go/3_ulaw_to_s16_2.go:3\\n u_val = ^u_val //4\\n var t <type -3> = ((($convert(<type -3>, (u_val & $convert(<type -5>, 15 ))) << $convert(<type -12>, 3 )) | 132 ) << ((u_val & $convert(<type -5>, 112 )) >> $convert(<type -12>, 4 ))) //6\\n { //8\\n  if ((u_val & $convert(<type -5>, 128 )) != $convert(<type -5>, 0 )) { //8\\n   { //9\\n    $ret0 = ($convert(<type -3>, 132 ) - t) //9\\n    return //9\\n   } //0\\n  } //8\\n } //10\\n { //12\\n  $ret0 = (t - $convert(<type -3>, 132 )) //12\\n  return //12\\n } //0\\n\22"
module asm "\09.ascii \22checksum 1821CD1979CE40EE6DA3F73E84A73062AF251E0D\\n\22"
module asm "\09.text"
...
then.8:                                           ; preds = %else.7
  ...
  %icmp.8.not = icmp eq i64 %tmpv.13.sroa.0.0.copyload, %tmpv.14.sroa.0.0.copyload
...
label.0:                                          ; preds = %then.21
  %add.0 = add nuw nsw i64 %tmpv.27.018, 1
...
then.21:                                          ; preds = %label.0, %entry.split
  %tmpv.27.018 = phi i64 [ %add.0, %label.0 ], [ 0, %entry.split ]
  ...
  %icmp.19 = icmp eq i32 %tmpv.29.sroa.0.0.copyload, %tmpv.30.sroa.0.0.copyload
  %icmp.20 = icmp eq i64 %tmpv.29.sroa.3.0.copyload, %tmpv.30.sroa.3.0.copyload
  %tmpv.36.0.in = select i1 %icmp.19, i1 %icmp.20, i1 false
  %icmp.21 = icmp eq i64 %tmpv.29.sroa.4.0.copyload, %tmpv.30.sroa.4.0.copyload
  %tmpv.37.0.in = select i1 %tmpv.36.0.in, i1 %icmp.21, i1 false
...
label.0:                                          ; preds = %then.36
  %add.1 = add nuw nsw i64 %tmpv.54.011, 1
...
label.0.1:                                        ; preds = %label.0
  %add.1.1 = add nuw nsw i64 %tmpv.54.011, 2
...
label.0.2:                                        ; preds = %label.0.1
  %add.1.2 = add nuw nsw i64 %tmpv.54.011, 3
...
then.36:                                          ; preds = %label.0.2, %entry.split
  %tmpv.54.011 = phi i64 [ %add.1.2, %label.0.2 ], [ 0, %entry.split ]
...
then.41:                                          ; preds = %then.44
  %icmp.47 = icmp eq ptr %tmpv.70.sroa.0.0.copyload, %tmpv.72.sroa.0.0.copyload
...
then.44:                                          ; preds = %else.43, %entry.split
  ...
  %icmp.46 = icmp eq i64 %tmpv.70.sroa.3.0.copyload, %tmpv.72.sroa.3.0.copyload
