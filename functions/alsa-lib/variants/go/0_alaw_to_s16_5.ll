; ModuleID = '/home/javier/Galapagos/functions/alsa-lib/variants/go/0_alaw_to_s16_5.bc'
...
module asm "\09.ascii \22)\22"
module asm "\09.ascii \22 <inl:742>\\n\22"
module asm "\09.ascii \22 // /home/javier/Galapagos/functions/alsa-lib/variants/go/0_alaw_to_s16_5.go:3\\n aVal = (aVal ^ $convert(<type -20>, 85 )) //4\\n var t <type -3> = $convert(<type -3>, (aVal & $convert(<type -20>, 127 ))) //5\\n { //6\\n  if (t < $convert(<type -3>, 16 )) { //6\\n   t = ((t << $convert(<type -12>, 4 )) + $convert(<type -3>, 8 )) //7\\n  } else { //8\\n   var seg <type -3> = ((t >> $convert(<type -12>, 4 )) & 7 ) //9\\n   t = ($convert(<type -3>, 264 ) + ((t & $convert(<type -3>, 15 )) << $convert(<type -12>, 4 ))) //10\\n   t = (t << (seg - $convert(<type -3>, 1 ))) //11\\n  } //6\\n } //12\\n { //13\\n  $ret0 = (t * $convert(<type -3>, ((($convert(<type -20>, 2 ) * aVal) & $convert(<type -20>, 128 )) - $convert(<type -20>, 1 )))) //13\\n  return //13\\n } //0\\n\22"
module asm "\09.ascii \22checksum 379486EF2EEF94ABF6AFC70CAC5924A966C21F63\\n\22"
module asm "\09.text"
...
entry:
  %xor.0 = xor i8 %aVal, 85
  %iand.0 = and i8 %xor.0, 127
...
fallthrough.0:                                    ; preds = %else.0, %then.0
  ...
  %mul.0 = shl i8 %xor.0, 1
  %iand.3 = and i8 %mul.0, -128
  %sub.1 = add i8 %iand.3, -1
  %zext.4 = zext i8 %sub.1 to i32
  %mul.1 = mul i32 %t.0, %zext.4
  ret i32 %mul.1
...
then.9:                                           ; preds = %else.8
  ...
  %icmp.9.not = icmp eq i64 %tmpv.12.sroa.0.0.copyload, %tmpv.13.sroa.0.0.copyload
...
then.22:                                          ; preds = %label.0, %entry.split
  ...
  %icmp.20 = icmp eq i32 %tmpv.28.sroa.0.0.copyload, %tmpv.29.sroa.0.0.copyload
  %icmp.21 = icmp eq i64 %tmpv.28.sroa.3.0.copyload, %tmpv.29.sroa.3.0.copyload
  %tmpv.35.0.in = select i1 %icmp.20, i1 %icmp.21, i1 false
  %icmp.22 = icmp eq i64 %tmpv.28.sroa.4.0.copyload, %tmpv.29.sroa.4.0.copyload
  %tmpv.36.0.in = select i1 %tmpv.35.0.in, i1 %icmp.22, i1 false
...
then.42:                                          ; preds = %then.45
  %icmp.48 = icmp eq ptr %tmpv.69.sroa.0.0.copyload, %tmpv.71.sroa.0.0.copyload
...
then.45:                                          ; preds = %else.44, %entry.split
  %tmpv.67.019 = phi i64 [ %add.4, %else.44 ], [ 0, %entry.split ]
  ...
  %icmp.47 = icmp eq i64 %tmpv.69.sroa.3.0.copyload, %tmpv.71.sroa.3.0.copyload
