; ModuleID = '/home/javier/Galapagos/functions/liboqs/variants/go/0_fpr_lt_8.bc'
...
module asm "\09.ascii \22)\22"
module asm "\09.ascii \22 <inl:567>\\n\22"
module asm "\09.ascii \22 // /home/javier/Galapagos/functions/liboqs/variants/go/0_fpr_lt_8.go:3\\n var sameSign <type -4> = ((x ^ y) >> $convert(<type -12>, 63 )) //4\\n var sy <type -4> = y //5\\n { //6\\n  if (sameSign != $convert(<type -4>, 0 )) { //6\\n   sy = x //7\\n  } //6\\n } //8\\n var cc0 <type -3> = ($convert(<type -3>, ((x - sy) >> $convert(<type -12>, 63 ))) & 1 ) //9\\n var cc1 <type -3> = ($convert(<type -3>, ((sy - x) >> $convert(<type -12>, 63 ))) & 1 ) //10\\n { //11\\n  $ret0 = (cc0 ^ ((cc0 ^ cc1) & $convert(<type -3>, ((x & y) >> $convert(<type -12>, 63 ))))) //11\\n  return //11\\n } //0\\n\22"
module asm "\09.ascii \22checksum 4671ECFA047AA68E803FDBD51FB49382D3BB2708\\n\22"
module asm "\09.text"
...
entry:
  %xor.0 = xor i64 %y, %x
  %icmp.0.not = icmp sgt i64 %xor.0, -1
  %spec.select = select i1 %icmp.0.not, i64 %y, i64 %x
  %sub.0 = sub i64 %x, %spec.select
  %shr.114 = lshr i64 %sub.0, 63
  %trunc.1 = trunc i64 %shr.114 to i32
  %sub.1 = sub i64 %spec.select, %x
  %shr.215 = lshr i64 %sub.1, 63
  %iand.2 = and i64 %y, %x
  %xor.116 = xor i64 %shr.114, %shr.215
  %isneg = icmp slt i64 %iand.2, 0
  %0 = trunc i64 %xor.116 to i32
