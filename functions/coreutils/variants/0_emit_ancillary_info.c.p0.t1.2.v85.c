Variant 1:
Make the functionality case sensitive (originally was case-insensitive).
...
Variant 2:
Make an efficient search in the map program by making it an unordered_map:
...
emit_ancillary_info (char const *program)
{
  ...
  char const *node = program;
  std::string strProg = std::string(program);
  auto iterator = infomap.find(strProg);
  
  if (iterator != infomap.end())
    node = iterator->second.c_str();
