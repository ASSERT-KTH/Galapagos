; ModuleID = '/home/javier/Galapagos/functions/ffmpeg/variants/go/0_flac_get_max_frame_size_7.bc'
...
module asm "\09.ascii \22<type -3>\22"
module asm "\09.ascii \22)\22"
module asm "\09.ascii \22 <inl:653>\\n\22"
module asm "\09.ascii \22 // /home/javier/Galapagos/functions/ffmpeg/variants/go/0_flac_get_max_frame_size_7.go:3\\n var count <type -3> = 16  //4\\n var perCh <type -3> = (ch * (((7  + bps) + 7 ) / 8 )) //5\\n var frameSz <type -3> //6\\n { //7\\n  if (ch == $convert(<type -3>, 2 )) { //7\\n   frameSz = ((((($convert(<type -3>, 2 ) * bps) + $convert(<type -3>, 1 )) * blocksize) + $convert(<type -3>, 7 )) / $convert(<type -3>, 8 )) //8\\n  } else { //9\\n   frameSz = ((((ch * bps) * blocksize) + $convert(<type -3>, 7 )) / $convert(<type -3>, 8 )) //10\\n  } //7\\n } //11\\n count = (count + ((perCh + frameSz) + $convert(<type -3>, 2 ))) //12\\n { //13\\n  $ret0 = count //13\\n  return //13\\n } //0\\n\22"
module asm "\09.ascii \22checksum 8FFA0B0819F1B0D3DC7C6193218BAFD326B771EA\\n\22"
module asm "\09.text"
...
entry:
  ...
  %frameSz.0.in.in = mul i32 %add.2.pn, %blocksize
  %frameSz.0.in = add i32 %frameSz.0.in.in, 7
  %frameSz.0 = sdiv i32 %frameSz.0.in, 8
  %add.1 = add i32 %bps, 14
  %div.0 = sdiv i32 %add.1, 8
  %mul.0 = mul i32 %div.0, %ch
  %add.5 = add i32 %mul.0, 18
  %add.7 = add i32 %add.5, %frameSz.0
  ret i32 %add.7
}
