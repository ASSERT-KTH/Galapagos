; ModuleID = '/home/javier/Galapagos/functions/ffmpeg/variants/go/0_flac_get_max_frame_size_1.bc'
...
module asm "\09.ascii \22 // /home/javier/Galapagos/functions/ffmpeg/variants/go/0_flac_get_max_frame_size_1.go:3\\n var count <type -3> = 16  //4\\n var headSize <type -3> = (ch * ((bps + 14 ) / 8 )) //5\\n { //6\\n  if (ch == $convert(<type -3>, 2 )) { //6\\n   var frameSize <type -3> = (((((2  * bps) + 1 ) * blocksize) + 7 ) / 8 ) //7\\n   count = (count + (headSize + frameSize)) //8\\n  } else { //9\\n   var frameSize <type -3> = ((((ch * bps) * blocksize) + 7 ) / 8 ) //10\\n   count = (count + (headSize + frameSize)) //11\\n  } //6\\n } //12\\n count = (count + $convert(<type -3>, 2 )) //13\\n { //14\\n  $ret0 = count //14\\n  return //14\\n } //0\\n\22"
module asm "\09.ascii \22checksum F89AC536E6F569D156FEE7D1A4DEFE474DE35666\\n\22"
module asm "\09.text"
...
entry:
  ...
  %div.1.pn.in.in = mul i32 %add.1.pn, %blocksize
  %div.1.pn.in = add i32 %div.1.pn.in.in, 7
  %div.1.pn = sdiv i32 %div.1.pn.in, 8
  %count.0.in = add i32 %mul.0, 18
  %add.8 = add i32 %count.0.in, %div.1.pn
  ret i32 %add.8
}
