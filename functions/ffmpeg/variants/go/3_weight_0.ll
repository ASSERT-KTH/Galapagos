; ModuleID = '/home/javier/Galapagos/functions/ffmpeg/variants/go/3_weight_0.bc'
...
entry:
  %mul.5 = shl i32 %offset, 1
  %icmp.9 = icmp sgt i32 %mul.5, %i
  br i1 %icmp.9, label %then.0, label %else.0

then.0:                                           ; preds = %entry
  %icmp.3 = icmp eq i32 %offset, 1
...
common.ret:                                       ; preds = %fallthrough.8, %then.6, %fallthrough.3, %then.1, %else.0
  ...
  %sub.11 = sub i32 %sub.10, %mul.5
  %icmp.8 = icmp slt i32 %sub.11, %i
  br i1 %icmp.8, label %then.5, label %common.ret

then.1:                                           ; preds = %then.0
  %icmp.0.not = icmp eq i32 %i, 0
  %. = select i1 %icmp.0.not, i32 3, i32 5
  br label %common.ret

else.1:                                           ; preds = %then.0
  %mul.0 = mul i32 %i, 6
  %add.0 = add i32 %mul.0, %offset
  %icmp.1 = icmp eq i32 %mul.5, 0
  br i1 %icmp.1, label %then.3, label %else.4
...
fallthrough.3:                                    ; preds = %else.4, %then.3
  ...
  %sub.1 = add i32 %mul.5, -1
  %sub.0 = add i32 %add.0, -1
  %div.0 = sdiv i32 %sub.0, %sub.1
  br label %fallthrough.3

then.5:                                           ; preds = %else.0
  %icmp.7 = icmp eq i32 %offset, 1
  br i1 %icmp.7, label %then.6, label %else.6

then.6:                                           ; preds = %then.5
  %icmp.4.not = icmp eq i32 %sub.10, %i
  %.34 = select i1 %icmp.4.not, i32 3, i32 5
  br label %common.ret

else.6:                                           ; preds = %then.5
  %sub.6 = sub i32 %sub.10, %i
  %mul.2 = mul i32 %sub.6, 6
  %add.2 = add i32 %mul.2, %offset
  %icmp.5 = icmp eq i32 %mul.5, 0
  br i1 %icmp.5, label %then.8, label %else.9
