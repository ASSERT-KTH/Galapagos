; ModuleID = '../functions/openssl/1_BitDeinterleave.once.bc'
source_filename = "/home/javier/Galapagos/functions/openssl/1_BitDeinterleave.run.once.c"
...
entry:
  %icmp.0.not = icmp eq i64 %Ai, 0
  br i1 %icmp.0.not, label %fallthrough.0, label %then.0
...
fallthrough.0:                                    ; preds = %then.0, %entry
  %Ai.addr.0 = phi i64 [ %ior.18, %then.0 ], [ 0, %entry ]
  ret i64 %Ai.addr.0
}
