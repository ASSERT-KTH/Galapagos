static void fiat_id_tc26_gost_3410_2012_512_paramSetB_addcarryx_u32(
    uint32_t *out1, fiat_id_tc26_gost_3410_2012_512_paramSetB_uint1 *out2,
    fiat_id_tc26_gost_3410_2012_512_paramSetB_uint1 arg1, uint32_t arg2,
    uint32_t arg3) {
    uint64_t result;
    uint32_t lower_result;
    fiat_id_tc26_gost_3410_2012_512_paramSetB_uint1 overflow;
    result = ((auto) arg1) + (uint64_t)arg2 + arg3;
    lower_result = (uint32_t) (result % (1ULL << 32));
    overflow = (fiat_id_tc26_gost_3410_2012_512_paramSetB_uint1)(result >= (1ULL << 32));
    *out1 = lower_result;
    *out2 = overflow;
}