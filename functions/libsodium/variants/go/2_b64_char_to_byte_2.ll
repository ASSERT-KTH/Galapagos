; ModuleID = '/home/javier/Galapagos/functions/libsodium/variants/go/2_b64_char_to_byte_2.bc'
...
entry:
  %0 = add i32 %c, -65
  %1 = icmp ult i32 %0, 26
  br i1 %1, label %common.ret, label %label.0

common.ret:                                       ; preds = %label.3, %else.5, %else.3, %entry
  %common.ret.op = phi i32 [ %add.0, %else.3 ], [ %add.1, %else.5 ], [ %switch.select32, %label.3 ], [ %0, %entry ]
...
label.3:                                          ; preds = %label.2
  ...
  %switch.selectcmp31 = icmp eq i32 %c, 43
  %switch.select32 = select i1 %switch.selectcmp31, i32 62, i32 %switch.select
  br label %common.ret
}
