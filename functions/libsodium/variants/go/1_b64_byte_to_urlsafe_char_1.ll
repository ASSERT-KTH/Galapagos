; ModuleID = '/home/javier/Galapagos/functions/libsodium/variants/go/1_b64_byte_to_urlsafe_char_1.bc'
...
label.1:                                          ; preds = %label.3, %else.2, %else.1, %else.0
  %char.0 = phi i32 [ %add.2, %else.2 ], [ %add.1, %else.1 ], [ %add.0, %else.0 ], [ %switch.select15, %label.3 ]
  ret i32 %char.0
...
label.3:                                          ; preds = %label.2
  ...
  %switch.selectcmp14 = icmp eq i32 %x, 62
  %switch.select15 = select i1 %switch.selectcmp14, i32 45, i32 %switch.select
  br label %label.1
}
