; ModuleID = '/home/javier/Galapagos/functions/libsodium/variants/go/0_b64_byte_to_char_2.bc'
...
module asm "\09.ascii \22)\22"
module asm "\09.ascii \22 <inl:1012>\\n\22"
module asm "\09.ascii \22 // /home/javier/Galapagos/functions/libsodium/variants/go/0_b64_byte_to_char_2.go:3\\n { //4\\n  if (x <= $convert(<type -7>, 25 )) { //4\\n   { //5\\n    $ret0 = $convert(<type -3>, (x + $convert(<type -7>, 65' ))) //5\\n    return //5\\n   } //0\\n  } else { //6\\n   { //6\\n    if (x <= $convert(<type -7>, 51 )) { //6\\n     { //7\\n      $ret0 = $convert(<type -3>, ((x - $convert(<type -7>, 26 )) + $convert(<type -7>, 97' ))) //7\\n      return //7\\n     } //0\\n    } else { //8\\n     { //8\\n      if (x <= $convert(<type -7>, 61 )) { //8\\n       { //9\\n        $ret0 = $convert(<type -3>, ((x - $convert(<type -7>, 52 )) + $convert(<type -7>, 48' ))) //9\\n        return //9\\n       } //0\\n      } else { //10\\n       { //10\\n        if (x == $convert(<type -7>, 62 )) { //10\\n         { //11\\n          $ret0 = $convert(<type -3>, 43' ) //11\\n          return //11\\n         } //0\\n        } //10\\n       } //12\\n      } //8\\n     } //12\\n    } //6\\n   } //12\\n  } //4\\n } //12\\n { //13\\n  $ret0 = $convert(<type -3>, 47' ) //13\\n  return //13\\n } //0\\n\22"
module asm "\09.ascii \22checksum 12D2B84F69FAB923DF39B13EFDF5C38D0AAB64C2\\n\22"
module asm "\09.text"
...
common.ret:                                       ; preds = %else.2, %then.2, %then.1, %then.0
  %common.ret.op = phi i32 [ %add.0, %then.0 ], [ %add.1, %then.1 ], [ %add.2, %then.2 ], [ %., %else.2 ]
...
then.2:                                           ; preds = %else.1
  ...
  %. = select i1 %icmp.0, i32 43, i32 47
  br label %common.ret
}
