; ModuleID = '/home/javier/Galapagos/functions/libsodium/variants/go/0_b64_byte_to_char_1.bc'
...
common.ret:                                       ; preds = %label.3, %else.2, %else.1, %else.0
  %common.ret.op = phi i32 [ %add.0, %else.0 ], [ %add.1, %else.1 ], [ %add.2, %else.2 ], [ %., %label.3 ]
...
label.3:                                          ; preds = %label.2
  %icmp.9.not = icmp eq i32 %x, 62
  %. = select i1 %icmp.9.not, i32 43, i32 47
  br label %common.ret
}
