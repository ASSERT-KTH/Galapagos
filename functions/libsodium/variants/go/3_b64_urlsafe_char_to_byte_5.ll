; ModuleID = '/home/javier/Galapagos/functions/libsodium/variants/go/3_b64_urlsafe_char_to_byte_5.bc'
...
label.1:                                          ; preds = %label.3, %else.5, %else.3, %else.1
  %result.0 = phi i64 [ %sext.226, %else.5 ], [ %sext.125, %else.3 ], [ %sext.024, %else.1 ], [ %switch.select23, %label.3 ]
  ret i64 %result.0

else.3:                                           ; preds = %label.0
  %add.0 = add nsw i32 %c, -71
  %sext.125 = zext i32 %add.0 to i64
  br label %label.1
...
label.2:                                          ; preds = %label.0
  ...
  %sext.226 = zext i32 %add.1 to i64
  br label %label.1

label.3:                                          ; preds = %label.2
  %switch.selectcmp = icmp eq i32 %c, 95
  %switch.select = select i1 %switch.selectcmp, i64 63, i64 0
  %switch.selectcmp22 = icmp eq i32 %c, 45
  %switch.select23 = select i1 %switch.selectcmp22, i64 62, i64 %switch.select
  br label %label.1
}
