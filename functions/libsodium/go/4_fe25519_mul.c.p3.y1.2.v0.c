...
(It should be noted that uint128 is not a standard Go type and custom implementation needs to be provided. Go’s standard library does have "math/big" which can hold big numbers but is slower due to dynamic memory allocation during each operation.)